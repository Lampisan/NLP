...
\item
Look at the other people mentioned in the tweet and find out if they have a similar profession. Use the hashtags. Crawl a dataset of tweets that mention the celebrities by handle. Get a list of celebrities to twitter handles. Now we can train a model to identify which celebrity was mentioned in a tweet (mask out the handle of this person).
\end{enumerate}
...
\begin{enumerate}
\item  
	Consider larger knowledge bases in the web (wikipedia, ...), other bachelor thesis,master thesis and other publications. If we know the field we can narrow down the data needed. \\ Previously known cases of plagiarism will also be useful.
\item 
	Use some vector embedding like GloVe. Compute similarity features and containment. Find subsequences that are the same (long = probably plagiarism)
\item 
	some deep learning model
\item
	Data collection would need large amount of resources. Further training the model on a large amount we would take quite some time.
\end{enumerate}
...
\begin{enumerate}
\item  
	machine learning approach, deep learning BERT (maybe we find a pretrained BERT-model that is trained on technical reports or similar), 
\item 
...
\begin{enumerate}
\item  
	somehow create some ground truth that we can compare to, otherwise we need to involve humans that rate the output of the model	
\item 
	compare the stylography of the output of the model with texts from the author we are trying to mimic
\item 
